...
shown below. The notation is based on \cite{hyndman2021}: $\ell$ denotes
the level and $b$ the trend or slope; $\alpha$ and $\beta$ are the
...
We'll start with a simple example [FIXME FIND ONE!]
replication of the Australian tourism example given by
\cite{hyndman2021}. Here's the hansl
code:\label{pg:ex1}
\begin{nscode}
open XXX.gdt
dataset addobs 12
...
% The \dtk{XXX} dataset contains 80 quarterly observations from
...
\begin{nscode}
open XXX.gdt #--frompkg=Winters
bundle wb = Holt(y, _(ntest=8))
\end{nscode}
...
\begin{figure}[htpb]
  \centering
  \includegraphics[scale=0.65]{Holt_dialog}
