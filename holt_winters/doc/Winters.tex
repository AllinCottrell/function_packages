...
following elements:
\begin{itemize}
\item \texttt{theta}: a 3-vector holding the smoothing parameter values.
...
Listing~\ref{list:compat} shows some cases based on the sample script
for version 1.1 of the package along with the revised code required to
produce similar results from version 2.0. Note that while in version
1.1 the \texttt{Winters} arguments are strictly positional, in version
2.0 they are in effect named, since they appear as elements of a gretl
bundle (apart from the first required series argument). And the
meaning of ``default smoothing parameters'' has changed: in version
1.1 that means using a fixed parameter vector,
$\theta = (0.3, 0.1, 0.7)$, while in version 2.0 it means that the
parameters are automatically optimized, along with the initializer.

In Listing~\ref{list:compat} we could have had version 2.0 replicate
the results using the default $\theta$ from version 1.1, but it seems
more informative to compare that default with optimized results.  In
Case 1, with a specified parameter vector, the results are the same
for the two versions, namely RMSE = 74.792 and MAE = 73.132 for the
testing forecasts. In Case 2, the default parameter vector in 1.1
gives a slight improvement: RMSE = 74.033, MAE = 70.341. The optimized
parameters with 2.0 give a more substantial improvement: RMSE =
65.582, MAE = 60.895.

In Case 3, with genuine out-of-sample forecasts, we're obviously not
in a position to offer forecast evaluation statistics.
...
\end{scode}
  \caption{Winters 1.1 examples with updates for 2.0}
  \label{list:compat}
\end{script}

The output data in the returned bundle also differ slightly across
the versions of the package. In version 1.11 four series are included:
the original series, the one-step ahead forecast, level-plus-trend,
and seasonal. Version 2.0 gives a five-column matrix holding the
original series, level, trend, seasonal, and forecast (as in
\cite{hyndman2021}).
