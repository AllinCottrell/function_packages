...
for classification mode, \texttt{tune} and \texttt{verbose}.
...
age of the passenger in years) is a plausible predictor of survival
but unfortunately this datum is missing in 263 cases. At present
missing values are not allowed in the sample passed to the package,
and we have omitted \texttt{age} from the specification. However, to
include \texttt{age} one could first sub-sample to omit any missing
observations then pass the reduced sample to \texttt{random\_forest}.
