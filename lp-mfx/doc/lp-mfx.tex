...
\usepackage[pdftex,hyperindex=false,hyperfootnotes=false]{hyperref}
...
\title{The \textsf{lp-mfx} package, version 1.1}
...
\section{Primary and secondary regressors}
...
of secondary (non-primary) regressors, namely
\begin{itemize}
...
\end{align*}
while the secondary terms, $x_1^2$ and $x_2 x_3$, do not possess
marginal effects of their own.

To ensure that such secondary terms are detected correctly, it is
...
If you are using a third-party dataset that includes secondary terms
...
Note that more elaborate cases of secondary regressors are not
