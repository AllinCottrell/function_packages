...
\item The \texttt{Wald}, \texttt{Htest} and \texttt{Stest} matrices,
  if present, are row vectors containing test statistic, degrees of
  freedom and $P$-value pertaining to the Wald, Hausman and Sargan
  ...
  truly has a non-zero coefficient. The Hausman test (null hypothesis:
  the Hausman--Taylor estimates are consistent) is based on a vector
  of contrasts, the difference between the fixed-effects and
  Hausman--Taylor estimates of the coefficients on the time-varying
  regressors. This test is shown provided the covariance matrix of
  this vector is positive-definite.  The Sargan (over-identification)
  test is shown only if there are more time-varying exogenous
  regressors than time-invariant endogenous ones; the null hypothesis
  is that all the instruments are valid.
\end{itemize}
