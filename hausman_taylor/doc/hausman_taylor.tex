...
\end{equation}
\item Perform an IV regression of $y_{it}^* = (y_{it} -
...
The final step can also be described thus: the quasi-demeaned
dependent variable is regressed on the quasi-demeaned regressors,
taking as instruments the fully-demeaned time-varying regressors, the
means of the exogenous time-varying terms, and the levels of the
exogenous time-invariant terms.
...
calculation of $\hat{\sigma}^2_u$ uses the harmonic mean of the
...
bundle hausman_taylor (series y "dependent variable", 
                       list L1 "exogenous regressors",
                       list L2 "endogenous regressors", 
                       int verbosity[0:2:1],
                       bool as_stata[0])
\end{code}

The series \texttt{y} is the dependent variable, and the lists
\texttt{L1} and \texttt{L2} correspond, respectively, to $x_1$ plus
...
\texttt{L1} and \texttt{L2}---but it can easily determine which
...
\item The \texttt{Htest} and \texttt{Stest} matrices, if present, are
  ...
  $P$-value pertaining to the Hausman and Sargan test, respectively.
  The Hausman test (null hypothesis: the Hausman--Taylor estimates are
  consistent) is based on a vector of contrasts, the difference between
  the fixed-effects and Hausman--Taylor estimates of the coefficients on
  the time-varying regressors. This test is shown provided the
  covariance matrix of the difference vector is positive-definite.  The
  Sargan (over-identification) test is shown only if there are more
  time-varying exogenous regressors than time-invariant endogenous ones;
  the null hypothesis is that all the instruments are valid.
\end{itemize}
...
\begin{tabular}{llp{.6\textwidth}}
  ...
  \texttt{xnames} & strings & names of regressors \\
  \texttt{L1} & list & exogenous regressors \\
  \texttt{L2} & list & endogenous regressors \\
...
Hausman--Taylor, and were revisited by \cite{baltagi-akom90} and in
...
Hausman test: chi-square(9) = 14.5555 [0.1039]
Sargan over-identification test: chi-square(3) = 14.8759 [0.0019]
\end{outbit}
...
Hausman test: chi-square(9) = 5.25773 [0.8113]
Sargan over-identification test: chi-square(3) = 5.2291 [0.1558]
\end{outbit}
\end{script}

In each case we may take it that the endogenous regressor of primary
interest is \texttt{ed} (education level). The specifications differ
in their treatment of two pairs of regressors: in Cornwell and Rupert
\texttt{wks} (weeks worked) and \texttt{ms} (marital status) are taken
to be exogenous while \texttt{occ} (blue-collar dummy) and
\texttt{ind} (manufacturing dummy) are endogenous; Baltagi assumes
that \texttt{wks} and \texttt{ms} are endogenous, \texttt{occ} and and
\texttt{ind} exogenous. Baltagi finds a slightly smaller, but much
more sharply estimated, return to education. The Hausman and Sargan
specification tests would seem to favor Baltagi's specification.
...
One further point: since $\sigma^2_u$ is estimated indirectly, it is
possible in finite samples to get a zero or negative value for
$\hat{\sigma}^2_u$, in which case the standard procedure is to set
$\theta = 0$. This destroys the distinction we're interested in,
between the two variants of the Hausman--Taylor estimator. It's
therefore necessary to calibrate the simulation so that a non-positive
$\hat{\sigma}^2_u$ doesn't arise too often.\footnote{In addition we
  discarded iterations in which $\hat{\sigma}^2_u$ was non-positive,
  continuing until the specified number of replications was reached
  with non-zero $\theta$.}
...
for four values of $N$.  Here we see that the relative performance of
the variants strongly supports the contention made above. While
performance with respect $\beta_1$ and $\beta_2$ is virtually
identical, the $\beta_0$ and $\alpha$ estimates show \textit{much}
greater variance when $x_1^*$ is not included as an instrument;
indeed, the variance of the $\alpha$ estimates is such that they may
be useless in practice.

We're now in a position to explain the \dtk{as_stata} option for the
