...
$z_1$ and $x_2$ plus $z_2$ as shown in equation~(\ref{eq:htmod}).
This function does not undertake to judge which regressors are
exogenous and which endogenous---you must decide the partition
between \texttt{L1} and \texttt{L2}---but it can easily determine
which regressors are time-varying and which invariant.
...
By way of return value, this function offers a bundle containing
...
We're now in a position to explain the \dtk{as_stata} option
for the \dtk{hausman_taylor} function mentioned on
